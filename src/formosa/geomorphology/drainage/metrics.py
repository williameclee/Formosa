...
def compute_flow_accumulation(
    ...
    wgts: NDArray[np.floating] | None = None,
    ...
    if wgts is not None:
        validate_same_shape(wgts, dirs, "weights", "flow direction rasters")
    else:
        wgts = np.ones_like(dirs, dtype=np.float64)
    ...
    if dsij is None:
        _, _, dsij, _ = compute_downstream_indices(
            dirs, dir_enc=dir_enc, valids=valids, check=False
        )
    ...
    match backend:
        case "python":
            orders = metrics_py.compute_flow_strahler_order(
                dirs=dirs, dir_enc=dir_enc, valids=valids, indegs=indegs
            )
        case "fortran":
...
def compute_dist2source(
    ...
    if indegs is None:
        indegs = count_indegree(dirs, dir_enc=dir_enc)
    else:
        validate_same_shape(indegs, dirs, "in-degree", "flow direction rasters")
