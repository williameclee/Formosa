...
def test_symbolic_infinite_face_uses_hull_visibility():
    ...
    assert tri_py.is_bad_triangle(inf_facet, 2, vtxs, iinf)
    assert tri_py.is_bad_triangle(inf_facet, 3, vtxs, iinf)
    assert not tri_py.is_bad_triangle(inf_facet, 4, vtxs, iinf)
...
def test_symbolic_infinite_predicate_rejects_invalid_cases():
    ...
    with pytest.raises(GraphTopologyError, match="cannot be inserted"):
        tri_py.is_bad_triangle((iinf, 1, 0), iinf, vtxs, iinf)
