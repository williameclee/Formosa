...
| [`compute_prominence`](../src/formosa/geomorphology/terrain.py) | `.geomorphology.terrain.compute_prominence` | Computes topographic prominence, peak labels, and key saddle labels for a DEM. | Fortran only | [Direct](../tests/test_terrain_f.py) | Returns `(proms, peaks, saddles)`; supports direction schemes and validity masks. |
