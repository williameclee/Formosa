...
def create_flowline_plot_data(
    ...
    dsi, dsj, _, ds_valids = compute_downstream_indices(
        dirs, dir_enc=dir_enc, check=False, return_flat_index=False
    )
...
def construct_flowgraph(
...
) -> tuple[NDArray[np.int8], NDArray[NpCanonIndex], NDArray[NpCanonIndex]]:
    """
    ...
    IncompleteFlowGraphError
        If either endpoint of a selected directed edge is absent
    ...
    indegs = flowdir_m.count_indegree(dirs, dir_enc, valids=valids, backend=backend)
    ...
    match backend:
        ...
        case "fortran":
            narcs, nvtxs, arc_orders, vtxs, endpts, err_code = (
    ...
    endpts = endpts[:, :narcs].T.copy(order="C")
    vtxs = vtxs[:, :nvtxs].T.copy(order="C")
    ...
    dsi, dsj, has_valid_ds = _valid_flow_edges(dirs, valids, dir_enc)
    ...
    return (
        arc_orders,
        vtxs.astype(NpCanonIndex, order="C"),
        endpts.astype(NpCanonIndex, order="C"),
    )
